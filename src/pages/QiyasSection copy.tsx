...
export default function QiyasSection() {
  ...
  const countInCategory = (cat: SourceCategory) => selectedSources.filter((s) => s.testType === testType && getSourcesByCategory(testType, cat).some((src) => src.id === s.id)).length;

  return (
    <div className="animate-fade-in px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 text-center"><h1 className="section-title text-3xl text-white sm:text-4xl">قسم القدرات</h1><p className="mt-2 text-sm text-ink-300">ابدأ بالتأسيس، ثم أضف التدريب — النظام يوزّعها بشكل تتابعي: تأسيس أولاً ثم تدريب</p></div>
        ...
        {!showInputs ? (
          <>
            <SectionBlock category="foundation" testType={testType} sources={foundationSources} isExpanded={expandedSections.has('foundation')} onToggle={() => toggleSection('foundation')} selectedCount={countInCategory('foundation')} isSourceSelected={isSourceSelected} onToggleSource={toggleSource} />
...
interface SectionBlockProps { category: SourceCategory; testType: TestType; sources: Source[]; isExpanded: boolean; onToggle: () => void; selectedCount: number; isSourceSelected: (id: string) => boolean; onToggleSource: (id: string, name: string, testType: TestType) => void; }
function SectionBlock({ category, testType, sources, isExpanded, onToggle, selectedCount, isSourceSelected, onToggleSource }: SectionBlockProps) {
  ...
  return (
    <div className={`rounded-2xl border ${meta.border} bg-ink-850/60 overflow-hidden`}>
      <button onClick={onToggle} className="flex w-full items-center justify-between p-5 transition-colors hover:bg-ink-800/40">
        <div className="flex items-center gap-3"><div className={`grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br ${meta.bg} border border-ink-600/50`}><Icon className={`h-5 w-5 ${meta.color}`} /></div><div className="text-right"><div className="font-display text-lg font-bold text-white">{meta.label}</div><div className="text-xs text-ink-300">{sources.length} مصادر متاحة{selectedCount > 0 && <span className="text-gold-300"> · {selectedCount} محدد</span>}</div></div></div>
      ...
      {isExpanded && (<div className="grid gap-4 p-5 pt-0 sm:grid-cols-2 lg:grid-cols-3">
        {sources.map((source, i) => { const selected = isSourceSelected(source.id); return (
          <button key={source.id} onClick={() => onToggleSource(source.id, source.name, testType)} className={`group relative overflow-hidden rounded-xl border p-4 text-right transition-all animate-fade-up ${selected ? 'border-gold-400/60 bg-gradient-to-br from-gold-400/10 to-transparent shadow-glow-gold' : 'border-ink-700 bg-ink-850/70 hover:border-ink-500 hover:bg-ink-800/70'}`} style={{ animationDelay: `${i * 60}ms` }}>
            ...
            <div className="mb-2 text-xl">🧠</div><h3 className="mb-1 font-display text-sm font-bold text-white">{source.name}</h3><p className="text-[11px] leading-relaxed text-ink-300">{source.description}</p>
