...
export default function TahsiliSection() {
  ...
  const activeSource = testTypeSources[0]?.groupId ?? null;
  const toggleSubject = (sourceId: string, sourceName: string, subject: TahsiliSubject) => {
    const id = makeId(sourceId, subject);
    if (isSourceSelected(id)) { removeSource(id); return; }
    if (activeSource && activeSource !== sourceId) return;
    addSource(id, `${sourceName} - ${TAHSILI_SUBJECTS[subject]}`, testType, sourceId);
  };
  ...
  return (
    <div className="animate-fade-in px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 text-center"><h1 className="section-title text-3xl text-white sm:text-4xl">قسم التحصيلي</h1><p className="mt-2 text-sm text-ink-300">اختر مصدراً واحداً، ثم حدد المواد التي تريد مذاكرتها منه</p></div>
          ...
          !showSources ? (
            ...
            <>
              <div className="space-y-4">
                {TAHSILI_SOURCES.map((source, i) => { const isExpanded = expandedSource === source.id; const isActive = activeSource === source.id; const selectedCount = isActive ? countSubjectsForSource(source.id) : 0; return (
                  <div key={source.id} className={`rounded-2xl border overflow-hidden transition-all animate-fade-up ${isActive ? 'border-gold-400/40 bg-ink-850/60' : 'border-ink-700 bg-ink-850/60'}`} style={{ animationDelay: `${i * 60}ms` }}>
                    <button onClick={() => handleSourceClick(source.id)} className="flex w-full items-center justify-between p-5 transition-colors hover:bg-ink-800/40">
                      <div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br from-gold-300/10 to-gold-500/5 border border-ink-600/50"><BookMarked className="h-5 w-5 text-gold-300" /></div><div className="text-right"><div className="font-display text-lg font-bold text-white">{source.name}</div><div className="text-xs text-ink-300">{isActive ? `${selectedCount} مواد محددة` : 'اضغط لاختيار المواد'}</div></div></div>
